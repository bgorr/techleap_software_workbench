...
def setServoAngle(servo, angle):
    ...
    # pwm.ChangeDutyCycle(2. + (90+angle)/18.)
    # time.sleep(0.4)
    # pwm.ChangeDutyCycle(0)
    # pwm.stop()
    if(servo==panPin):
        az_setting = ('FP'+str(60*angle)+'\r').encode('UTF-8')
        ser_az.write(az_setting)
    if(servo==tiltPin):
        el_setting = ('FP'+str(60*angle)+'\r').encode('UTF-8')
        ser_el.write(el_setting)
    
    print ("Turned to angle: ", angle)
...
def main():
    ...
    while(True):
        ...
        if mode == MODE_TRACK and tracker_initialized:
            ...
            print("panAngle, tiltAngle requested: ", int(panAngle), int(tiltAngle))
            setServoAngle(panPin, int(panAngle))
            setServoAngle(tiltPin, int(tiltAngle))
...
if __name__ == '__main__':
    ...
    setServoAngle(panPin,0) # facing the object - initialization
    setServoAngle(tiltPin,-75)
    time.sleep(0.2)
    main()
    GPIO.cleanup()
    
    #setServoAngle(-0.031640625,-74.96944444444445)
